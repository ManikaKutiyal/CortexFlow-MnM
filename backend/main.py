...
PREFERRED_SAFETY_MODELS = [
    m.strip()
    for m in os.getenv(
        "GEMINI_SAFETY_CANDIDATES",
        "gemini-2.5-flash,gemini-2.0-flash",
    ).split(",")
    if m.strip()
]

OVERRIDE_REASONING_MODEL = os.getenv("GEMINI_REASONING_MODEL", "").strip()
OVERRIDE_SAFETY_MODEL = os.getenv("GEMINI_SAFETY_MODEL", "").strip()
    ...
    async with _MODEL_CACHE_LOCK:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=GEMINI_TIMEOUT_SECONDS) as client:
                ...
                res = await client.get(f"{GEMINI_API_BASE}/models", params={"key": GEMINI_API_KEY})
                ...
                models = sorted(

                    {
                        ...
                        str(item.get("name", "")).replace("models/", "", 1)
...
def pick_model(available: list[str], override: str, candidates: list[str]) -> Optional[str]:
    ...
    for m in available:
        ...
        if "flash" in lowered or "pro" in lowered:
...
async def gemini_chat(model: str, system: str, user: str, temperature: float = 0.2) -> Optional[str]:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=GEMINI_TIMEOUT_SECONDS) as client:
            ...
            res = await client.post(
                ...
                f"{GEMINI_API_BASE}/{model_path(model)}:generateContent",
...
async def compose_safe_summary(state: AnalysisState, risk_level: str) -> tuple[str, dict[str, Optional[str]]]:
    ...
    summary = await gemini_chat(reasoning_model, system, user, temperature=0.15)
    ...
    if safety_model:
        ...
        safe = await gemini_chat(safety_model, safety_system, safety_user, temperature=0.1)
...
async def models_recommended() -> dict[str, Any]:
    ...
    return {
        ...
        "notes": {
            "production_primary": "gemini-2.5-flash",
            "production_fallback": "gemini-2.0-flash",
            "fast_fallback": "gemini-2.0-flash",
        },
    }
